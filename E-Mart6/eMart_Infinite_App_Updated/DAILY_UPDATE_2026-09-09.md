...
## 2026-09-10 — v1.9.0 Production UI refinement
- Reworked the homepage hero into a restrained CSS 3D product stage using the project's existing real photographic imagery.
- Fixed smart-search selection so suggestions close and the selected product opens cleanly without stale suggestions remaining on screen.
- Removed native horizontal/side scrollbar visuals and added smooth product-rail scrolling.
- Added a compact Rail/Grid layout toggle to product sections.
- Added a real grid icon and responsive grid behavior.
- Kept the existing catalog, service routes, cart flow and real photography URLs intact.
