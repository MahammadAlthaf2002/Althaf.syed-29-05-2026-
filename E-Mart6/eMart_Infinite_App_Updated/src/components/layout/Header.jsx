...
export default function Header(){
 ...
 const search=()=>{if(q.trim()){setOpen(false);setQ('');nav(`/compare/search?q=${encodeURIComponent(q.trim())}`)}};
 const selectProduct=(id)=>{setOpen(false);setQ('');nav(`/product/${id}`)};
 return <>
  <header className="header">
   ...
   <div className="searchWrap">
    ...
    {q.trim()&&<div className="searchDropdown">
      ...
      {matches.map(p=><button className="searchResult" key={p.id} onClick={()=>selectProduct(p.id)}><img src={p.image} alt="" onError={e=>e.currentTarget.classList.add('imgFailed')}/><span><b>{p.name}</b><small>{p.tag} · ₹{p.price.toLocaleString('en-IN')}</small></span><Icon name="arrow" size={14}/></button>)}
