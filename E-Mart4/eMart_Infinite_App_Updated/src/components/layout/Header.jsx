...
export default function Header(){
 ...
 const [theme,setTheme]=useState(()=>localStorage.getItem('emart_theme')||'dark');
 useEffect(()=>{document.documentElement.dataset.theme=theme;localStorage.setItem('emart_theme',theme)},[theme]);
 ...
 const search=()=>{if(q.trim()){nav(`/compare/search?q=${encodeURIComponent(q.trim())}`);setQ('')}};
 return <>
  <header className="header">
   ...
   <div className="searchWrap">
    ...
    {q.trim()&&<div className="searchDropdown">
      ...
      {matches.map(p=><button className="searchResult" key={p.id} onClick={()=>{setQ('');nav(`/product/${p.id}`)}}><img src={p.image} alt="" onError={e=>e.currentTarget.classList.add('imgFailed')}/><span><b>{p.name}</b><small>{p.tag} · ₹{p.price.toLocaleString('en-IN')}</small></span><Icon name="arrow" size={14}/></button>)}
      <button className="searchAll" onClick={search}>View all results for “{q.trim()}” <Icon name="arrow" size={14}/></button>
    </div>}
   </div>
   <div className="headerActions"><button className="themeToggle" onClick={()=>setTheme(theme==='dark'?'light':'dark')} aria-label="Toggle theme"><span>{theme==='dark'?'☀':'◐'}</span><small>{theme==='dark'?'Warm light':'Dark mode'}</small></button>
